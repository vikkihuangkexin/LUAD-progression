...
clear all;
close all;
% % geneexpressionall = csvread('E:\LUAD_model\otherdata\validation\dataall.csv'); %LUAD所有的基因表达值（带正常人）
% % % geneexpression = csvread('E:\LUAD_model\otherdata\GSE26939_subtype.csv');%LUAD癌症患者的基因表达值（不带正常人）
% % d = geneexpressionall';
% % d=zscore(d);%做标准化
% % clusternortumor = csvread('E:\LUAD_model\otherdata\validation\subtype.csv');%亚型标签（0,1,2,3）,N*1
% % f = clusternortumor';
% % fea = mrmr_miq_d(d,f,60);%MRMR特征选择

cluster10 = csvread('E:\LUAD_model\otherdata\validation\cluster_iCluster.csv');%kmeans_cluster为kmeans的聚类结果
%cluster101 = csvread('E:\LUAD_model\otherdata\validation\kmeans_cluster.csv');%kmeans_cluster为kmeans的聚类结果
% cluster4 = find(cluster10==4);
% cluster10([cluster4],:)=[];%去除标签4
% geneexpressionall(:,[cluster4])=[];%去除基因表达矩阵中标签为4的样本
%%%%%%%%%%%%%%%%%%%%%%%  PCA可视化  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% data = geneexpressionall(fea,:);
data= csvread('E:\LUAD_model\otherdata\validation\data214.csv');
